@props(['title', 'konten', 'category', 'date', 'thumbnail', 'link'])
...
<div class="max-w-2xl overflow-hidden bg-white rounded-lg shadow-md dark:bg-gray-800">
    ...
    <div class="p-6">
        <div>
            <div>
                <span class="text-xs font-medium text-blue-600 uppercase dark:text-blue-400">{{ $category }}</span> |
                <span>{{ $date }}</span>
            </div>
            <a href="#"
                ...
                tabindex="0" role="link">{{ $title }}</a>
            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{!! Str::limit(strip_tags($description), 80) !!}</p>
        </div>
